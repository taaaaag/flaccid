...
@app.command("match")
def playlist_match(
    input_file: Path = typer.Argument(..., exists=True, dir_okay=False, help="Playlist file to match (e.g., playlist.json, .m3u, .csv)."),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Output file for the detailed JSON match report."),
):
    """
    ...
    try:
        ...
        console.print(f"✅ Match complete. Matched {matched_count}/{len(results)} tracks ({matched_count/len(results)*100:.1f}%)")
        exporter.export(results, output, "json")
        console.print(f"💾 Full match report saved to: [blue]{output}[/blue]")
        console.print("👉 You can now use the `export` command on the .match.json file.")
...
@app.command("export")
def playlist_export(
    match_file: Path = typer.Argument(..., exists=True, dir_okay=False, help="A .match.json report file generated by the `match` command."),
    format: str = typer.Option("m3u", "--format", "-f", help="Export format (m3u, json)."),
    ...
    try:
        ...
        for item in raw_results:
            try:
                ...
                results.append(MatchResult(input_track=pt, matched_track=matched, file_path=Path(file_path) if file_path else None))
            except Exception:
